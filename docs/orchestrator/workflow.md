...
| Agent | 角色 | 生命周期 | 职责 |
|-------|------|---------|------|
| **主 Agent** | 编排者 / 决策中心 | 整个项目 | 生成任务书、读所有报告、分析判定、写 deliverable（摘要/legacy/contracts） |
| **项目 Agent** | 执行者（手） | 单次调用，用完即弃 | 按任务书编码 + go build/vet，不决策不分析 |
| **QA Agent** | 执行者（手） | 单次调用，用完即弃 | 按 QA 协议测试 + 产出报告，不决策不分析 |
| **人** | 最终裁决者 | 整个项目 | Gate 1 审批需求、Gate 2 测试验收、处理主 Agent 无法判定的问题 |

**核心变化**：项目 Agent 和 QA Agent 是无状态的执行手。所有分析、判定、文件产出由主 Agent 完成。主 Agent 是唯一贯穿全流程的上下文持有者。

```
阶段 N 需求 md（人 + 主Agent 产出，status: approved）
     │
     ▼
主 Agent → 生成任务书 → 下派项目 Agent（手，用完即弃）
     │
     ▼
项目 Agent 编码 → go build ✅ → 交付 → 销毁
     │
     ▼
主 Agent → 读 diff → 写交付摘要 → 下派 QA Agent（手，用完即弃）
     │
     ▼
QA Agent 测试 → QA 报告 → 销毁
     │
     ▼
┌──────────────────────────────────────────────┐
│ 主 Agent 分析 QA 报告（决策中心）               │
│                                                │
│ ├── 全部通过，无 critical                       │
│ │     → 告知人："阶段 N QA 通过，请测试验收"     │
│ │     → 交人测试                                │
│ │                                              │
│ ├── 有可精确修复的 bug（≤20行、1文件、不改接口） │
│ │     → 生成精确修复任务                        │
│ │     → 新项目 Agent → 新 QA → 主 Agent 再判    │
│ │     → 最多 1 轮自动修复                       │
│ │                                              │
│ └── 根因不明 / 设计问题 / 回归 / 改接口         │
│       → 产出"问题分析摘要"                      │
...
└──────────────────────────────────────────────┘
     ...
     ▼
人
  ├── 读 QA 报告 + 主Agent 的 QA 摘要
  ├── 自己动手测试
  ├── 通过 → 锁版 → 主Agent 更新 contracts.md + 写 legacy.md → 阶段 N+1
  └── 不通过 → 测试文档写入 QA 报告（分界线以下）→ 主Agent 读人的文档 → 判定
                │
                ├── 精确 bug → 生成修复任务（最多 1 轮）
                └── 根因不明 → 口头讨论 → 人决策路径
```
...
> 项目 Agent 交付后即销毁。主 Agent 负责读 diff、产出交付摘要、下派 QA。
...
**修复约束**：项目 Agent 执行修复任务时必须遵守：

| 约束 | 说明 |
|------|------|
| **最小化 diff** | 单次修复 ≤ 20 行，1 个文件 |
| **不改无关代码** | 不得修改修复任务未指定的文件或函数 |
| **不改接口签名** | 修复不得改变已有 API 的字段名、类型、路径、方法 |

### 5.3 阶段 3：主 Agent 读 diff + 产出交付摘要
...
步骤：
1. 主 Agent 读项目 Agent 的代码 diff
2. 主 Agent 对比任务书，确认覆盖范围
3. 主 Agent 产出交付摘要：
   - 本阶段新建的包/文件
   - 本阶段修改的已有文件
   - 后续阶段不可碰的边界
...
### 5.4 阶段 4：QA Agent 测试

```
角色：QA Agent（手，不决策，用完即弃）
输入：需求 md + 代码 diff + contracts.md + regression-anchors.md
输出：QA 报告（结构化）

测试维度：
  - 回归：先跑回归锚点，任一失败直接驳回
  - 正确性：逐条对照验收条件，构造 Happy Path + Edge Case + Error Case
  - 质量：按 Go 专项检查清单 + 编码规范扫描

QA Agent 产出报告后销毁。
```

### 5.5 阶段 5：主 Agent 分析 QA 报告
...
决策树：
  ├── 全部通过（无 critical）
  │     → 产出"QA 摘要"告知人
  │     → 更新需求 md：status: qa-testing → human-testing
  │
  ├── 有可精确修复的 bug
  │     条件：≤20 行、1 个文件、不改接口、不涉及回归
  │     → 主 Agent 生成精确修复任务（具体到 Y 文件第 X 行改什么）
  │     → 新项目 Agent 执行修复 → 新 QA 复测 → 主 Agent 再判
  │     → 最多 1 轮自动修复
  │
  └── 根因不明 / 设计问题 / 回归 / 需改接口 / 跨包修改
        → 产出"问题分析摘要"
        → 口头告知人，人决策
```
...
```
角色：人
输入：QA 报告 + 主 Agent 的 QA 摘要
输出：锁版 or 反馈

步骤：
1. 人读 QA 报告 + 主 Agent 的摘要
2. 人自己动手测试（本地 Docker 环境）
3. 确认通过 → 锁版
   - 更新需求 md：status: human-testing → locked
   - 主 Agent 更新 contracts.md + 生成 stage-N-legacy.md
   - 进入阶段 N+1
4. 发现问题 → 写入测试文档（同份 QA 报告，分界线以下）
   - 更新需求 md：status: human-testing → fixing
   - 主 Agent 读人的测试文档 → 回到 5.5 决策树
```
...
| 可自主决定 | 必须交人决定 |
|-----------|------------|
| 单点 bug，位置明确，≤20 行，1 文件 | 需要改需求 md 或任务书 |
| 代码质量（命名/注释/重复） | 需要改 locked 接口签名 |
| QA 全部通过 + 告知人验收 | 回归——旧锚点被破坏 |
| 生成精确修复任务（1 轮） | 同个 bug 修 1 次还没解决 |
| | 涉及跨包、改架构、非局部修复 |
...
> 所有升级路径的决策点都在主 Agent。项目 Agent 和 QA Agent 不参与决策。
...
项目Agent 编码 → 编译预检失败
     │
     ├── 第 1-2 次失败：项目Agent 自行修复，重试
     ├── 第 3 次失败：升级到主Agent
     │     → 主Agent 判定：是任务书有歧义还是复杂度太高？
     │     → 任务书问题 → 重写任务书，新项目Agent 重新开始
     │     → 复杂度问题 → 口头告知人，人决策
     └── 超过 3 次：强制告知人
```
...
```
QA 报告 → 主 Agent 分析（见 5.5 决策树）
     │
     ├── 可精确修复 → 生成修复任务 → 新项目Agent → 新QA → 主Agent再判
     │     → 最多 1 轮
     │
     ├── 根因不明 / 设计问题 → 产出问题分析摘要 → 口头告知人
     │
     └── 回归（旧锚点失败）
           → ❌ 不得自动修复
           → 直接告知人
           → 人判定：是项目Agent 手滑？还是架构约束有问题？
```

### 7.3 人测试不通过

```
人测试发现问题 → 写入测试文档（QA 报告分界线以下）
     │
     ▼
主 Agent 读人的测试文档 → 回到 7.2 决策树（QA 发现问题）
     │
     ├── 主Agent 能精确定位 → 1 轮自动修复
     └── 主Agent 不能确定根因 → 口头讨论，人决策
```

### 7.4 已锁版阶段被新阶段破坏（回归）

```
主 Agent 发现 QA 报告中回归锚点失败
     │
     ▼
❌ 不得自动修复。口头告知人。
     │
     ▼
人决策：
  ├── 项目Agent 手滑 → 人审 diff，主Agent 生成精确回退 + 修复任务
  ├── 接口保护不够 → 主Agent 更新 contracts.md 约束 + legacy.md 边界
  └── 架构设计缺陷 → 人 + 主Agent 重新规划，可能回退阶段状态
```
...
后续阶段需要重写上游代码时：
- 接口不变、只改内部实现、回归锚点全过 → 主 Agent 可在任务书中批准
- 需要改 locked 接口签名 → 口头告知人，人批准后新增 `/v2/` 端点
- 技术债已在需求 md 中标明 → 主 Agent 直接纳入任务书

...
if not exist "nginx\ssl\server.crt" (
    ...
    docker run --rm -v "%CD%\nginx\ssl:/ssl" alpine:3.19 sh -c "apk add --no-cache openssl >/dev/null 2>&1 && openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /ssl/server.key -out /ssl/server.crt -subj '/C=CN/ST=Shanghai/L=Shanghai/O=B-B/OU=Dev/CN=localhost'" >nul 2>&1
    if %errorlevel% neq 0 (
